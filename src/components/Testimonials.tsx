      ...
      >
        {
            testimonial.filter((item) => item.service === props.selectedService).map((item:any, index:number) => (
                <SwiperSlide className='rounded-[40px] bg-white w-full md:!w-[400px] h-fit p-[10px] pb-[20px] flex flex-col space-y-[20px] md:space-y-[28px] items-center group/item cursor-pointer' key={index}>
                    <div className={`h-[275px] w-full rounded-[32px] ease-linear transition-colors duration-300 bg-linear-to-b from-[#99999980] to-[#999999] 
                    ${
                    item.company === 'git global' ? 'lg:group-hover/item:from-[#001F3D80] lg:group-hover/item:to-[#001F3D]' 
                    : 
                    item.company === 'JMG Handyman' ? 'lg:group-hover/item:from-[#DA232780] lg:group-hover/item:to-[#DA2327]' 
                    :
                    item.company === 'BlockChain Management' ? 'lg:group-hover/item:from-[#F4844C80] lg:group-hover/item:to-[#F4844C]' 
                    :
                    item.company === 'Weight Loss Paradise' ? 'lg:group-hover/item:from-[#31939380] lg:group-hover/item:to-[#319393]' 
                    :
                    item.company === 'BlockChain Management' ? 'lg:group-hover/item:from-[#F4844C80] lg:group-hover/item:to-[#F4844C]' 
                    :
                    item.company === 'Excel Daycare' ? 'lg:group-hover/item:from-[#69319380] lg:group-hover/item:to-[#693193]' 
                    :
                    ''
                    } 
                    relative`
                    ...
                    <div className="flex flex-col gap-[12px] px-[10px]">
                        {
                            item.reviews.map((review:any, index:number) => (
                                <div className={`bg-[#EDEFF7] rounded-[24px] odd:rounded-bl-[0px] even:rounded-br-[0px] w-fit even:self-end py-3 px-4 space-y-3 flex flex-col odd:items-start even:items-end even:bg-black lg:even:bg-[#EDEFF7] lg:group-hover/item:even:bg-black ease-linear transition-colors duration-300`} key={index}>
                                    <p className={`${inter.className} text-[14px]/[21px] md:text-[16px]/[24px] font-normal tracking-[-0.56px] text-black group-hover/item:text-white`}>
                                        {review.message}
                                    </p>
